            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading bg-red">
                        <h3 class="panel-title"><strong>تعديل حي</strong></h3>
                    </div>
                    <div class="panel-body">
                        <form action="{{url('districts', [$content->id])}}" method="post"  enctype="multipart/form-data">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select name="district_city_id" id="citySelect" class="form-control">
                                                <option value="{{ $content->district_city_id }}">{{ $content->districtCity ? $content->districtCity->cities_ar_name : null }}</option>
                                            </select>
                                        </div>
                                    </div>
                                   @include('.forms.inputText', ['label' => 'اسم الحي باللغه العربية ', 'name' => 'district_name_ar','content' => $content])
                                    @include('.forms.inputText', ['label' => 'اسم الحي باللغه الانجليزيه', 'name' => 'district_name_eng','content' => $content])
                                    @include('.forms.inputChekBox', ['label' => 'حالة التفعيل','checkboxLabel'=>'حالة التفعيل', 'name' => 'district_isactive','content' => $content])
                                </div>
                            </div>
                            <div class="row">
