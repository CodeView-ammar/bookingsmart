            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading bg-red">
                        <h3 class="panel-title"><strong>شاشة </strong> الاحياء</h3>
                    </div>
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12 m-b-20">
                                <div class="btn-group">
                                    <a href="{{url('districts/create')}}"  class="btn btn-danger">
                                ...
                                <div class="btn-group pull-right">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        ...
                                        @php
                                            $print = "
                                            <div class='text-center'>الاحياء</div>
                                             ...
                                             foreach($content as $key=>$item) {
                                                 $print .= "
                                                     <tr>
                                                         ...
                                                         <td>".  @$item->districtCity->citiesCountry->country_ar_name."</td>
                                                         <td>". @$item->districtCity->cities_ar_name  ."</td>
                                                         <td>".@$item->district_name_ar ."</td>
                                        ...
                                        @endphp
                                        <li><a id="" href="{{ url('pdf') }}?print={{ urlencode($print) }}&name=الاحياء.pdf">احفظ كملف pdf</a></li>                                        </li>
                                ...
                                @if($content->count())
                                    <table class="table table-striped table-hover dataTable" id="table-editable">
                                        ...
                                        <tr>
                                            <td width="35">{{ $key+1 }}</td>
                                            <td>{{ @$item->districtCity->citiesCountry->country_ar_name ?? " " }}</td>
                                            <td>{{ $item->districtCity ? $item->districtCity->cities_ar_name : null }}</td>

                                            <td>{{ $item->district_name_ar }}</td>
                                            <td>{{ $item->created_at }}</td>
                                            <td>{{ $item->district_isactive? 'مفعل':'غير مفعل' }}</td>

                                            <td class="text-center">
                                                <a style="color: white" class="edit btn btn-dark" href="{{ URL::to('districts/' . $item->id . '/edit') }}">
                                                    <i class="fa fa-pencil-square-o"></i>تعديل</a>
                                               {{-- <a class="delete btn btn-danger" href="javascript:;"><i class="fa fa-times-circle"></i>
                                                    Remove
                                                </a>--}}
                                                <form style="display: contents" action="{{ url('districts', [$item->id]) }}" method="POST">
...
@section('custom-scripts')
    ...
    <script>
        document.getElementById('export-btn').addEventListener('click', function () {
            ...
            // حفظ الملف
            saveAs(new Blob([s2ab(wbout)], { type: 'application/octet-stream' }), 'districts.xlsx');
        });
