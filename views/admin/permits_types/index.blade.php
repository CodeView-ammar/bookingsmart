            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading bg-red">
                        <h3 class="panel-title"><strong>شاشة </strong> انواع التصاريح</h3>
                    </div>
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12 m-b-20">
                                <div class="btn-group">
                                    <a href="{{url('permits_types/create')}}"  class="btn btn-danger">
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12 table-responsive table-red">
                                @if($content->count())
                                    <table class="table table-striped table-hover dataTable" id="table-editable">
                                        ...
                                        <tr role="row">
                                            <th>م</th>
                                            <th>نوع التصريح باللغه العربية </th>
                                            <th>اسم التصريح باللغه الانجليزية </th>
                                            <th>حالة التفعيل </th>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $item->Perm_name_ar }}</td>
                                            <td>{{ $item->Perm_name_eng }}</td>
                                            <td>{{  $item->Perm_isactive ? 'مفعل' : 'غير مفعل'  }}</td>

                                            <td class="text-center">
                                                <a class="edit btn btn-dark" style="color:white;" href="{{ URL::to('permits_types/' . $item->id . '/edit') }}">
                                                    <i class="fa fa-pencil-square-o" ></i>تعديل</a>
                                               {{-- <a class="delete btn btn-danger" href="javascript:;"><i class="fa fa-times-circle"></i>
                                                    Remove
                                                </a>--}}
                                                <form style="display: contents" action="{{ url('permits_types', [$item->id]) }}" method="POST">
...
@section('custom-scripts')
    ...
    <script>
        document.getElementById('export-btn').addEventListener('click', function () {
            ...
            // حفظ الملف
            saveAs(new Blob([s2ab(wbout)], { type: 'application/octet-stream' }), 'permits_types.xlsx');
        });
