            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading bg-red">
                        <h3 class="panel-title"><strong>  سياسات استخدام القاعات </strong></h3>
                    </div>
                    <div class="panel-body ">
                        <div class="row">
                            <div class="col-md-12 m-b-20 noPrint">
                                <div class="btn-group">
                                    <a href="{{url('usage_policies/create')}}"  class="btn btn-danger">
                                       اضف الان <i class="fa fa-plus"></i>
                                    </a>
                                </div>
                                <div class="btn-group pull-right">
                                    <button class="btn btn-default dropdown-toggle" data-toggle="dropdown">تصدير  <i class="fa fa-angle-down"></i>
                                    </button>
                                    ...
                                    <table class="table table-striped table-hover dataTable" id="table-editable">
                                        ...
                                        <tr>
                                            <td width="35">{{ $key+1 }}</td>
											 ...
											 <td>{{ $item->policiesHall ? $item->policiesHall->hall_code : null }}</td>
											  <td>{{ $item->policies_text }}</td>
                                            <td>{{ $item->policies_isactive?'مفعل':'غير مفعل' }}</td>

                                            <td class="text-center noPrint">
                                                <a style="color: white" class="edit btn btn-dark" href="{{ URL::to('usage_policies/' . $item->id . '/edit') }}">
                                                    <i class="fa fa-pencil-square-o"></i>تعديل</a>
                                               {{-- <a class="delete btn btn-danger" href="javascript:;"><i class="fa fa-times-circle"></i>
                                                    Remove
                                                </a>--}}
                                                <form style="display: contents" action="{{ url('usage_policies', [$item->id]) }}" method="POST">
...
@section('custom-scripts')
    ...
    <script>
        document.getElementById('export-btn').addEventListener('click', function () {
            ...
            // حفظ الملف
            saveAs(new Blob([s2ab(wbout)], { type: 'application/octet-stream' }), 'usage_policies.xlsx');
        });
