            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading bg-red">
                        <h3 class="panel-title"><strong>شاشة </strong> الادارات</h3>
                    </div>
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12 m-b-20">
                                <div class="btn-group">
                                    <a href="{{url('departments/create')}}"  class="btn btn-danger">
                                ...
                                <div class="btn-group pull-right">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        ...
                                        @php
                                            $print = "
                                            <div class='text-center'>الادارات</div>
                                                 <table class='table table-striped table-hover dataTable' id='table-editable'>
                                                     <thead>
                                                         <tr role='row'>
                                                             <th>م</th>
                                                             <th> اسم الادارة باللغه العربية</th>
                                                             <th>اسم الادارة باللغة الانجليزية </th>
                                                             <th>اسم المبني </th>
                                                             <th>الحي </th>
                                                             <th> تاريخ الانشاء </th>
                                                             <th> كود العميل </th>
                                             ...
                                             foreach($content as $key=>$item) {
                                                 $print .= "
                                                     <tr>
                                                         ...
                                                         <td>". $item->dept_name_ar ."</td>
                                                         <td>". $item->dept_name_eng ."</td>
                                                         <td>".  @$item->deptBuild->build_no ."</td>
                                                         <td>". @$item->deptBuild->buildDist->district_name_ar ."</td>
                                                         <td>".$item->created_at  ."</td>
                                                         <td>".$item->dept_cust_code   ."</td>
                                                     </tr>";
                                             }
                                        ...
                                        @endphp
                                        <li><a id="" href="{{ url('pdf') }}?print={{ urlencode($print) }}&name=الادارات.pdf">احفظ كملف pdf</a></li>
                                        </li>
                                ...
                                @if($content->count())
                                    <table class="table table-striped table-hover dataTable" id="table-editable">
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $item->dept_name_ar }}</td>
                                            <td>{{ $item->dept_name_eng }}</td>
                                            <td>{{ $item->deptBuild ? $item->deptBuild->build_no : null }}</td>
                                            <td>{{ @$item->deptBuild->buildDist->district_name_ar??" " }}</td>
                                            <td>{{ $item->created_at }}</td>
                                            <td>{{ $item->dept_cust_code }}</td>
                                            <td class="text-center">
                                                <a style="color: white" class="edit btn btn-dark" href="{{ URL::to('departments/' . $item->id . '/edit') }}">
                                                    <i class="fa fa-pencil-square-o"></i>تعديل</a>
                                               {{-- <a class="delete btn btn-danger" href="javascript:;"><i class="fa fa-times-circle"></i>
                                                    Remove
                                                </a>--}}
                                                <form style="display: contents" action="{{ url('departments', [$item->id]) }}" method="POST">
